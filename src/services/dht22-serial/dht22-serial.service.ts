...
class Dht22SerialDevice extends Device {
  address!: string;
  id!: string;
  interval!: number;
  parent!: string;
...
class Dht22SerialCommandData {
  slug!: string;
  value!: any;
}

@Injectable()
export class Dht22SerialService implements OnModuleDestroy, VirtualDevice {

  private appMessagesService!: AppMessagesService;
  private serialConnectorService!: SerialConnectorService;

  private readonly logger = new Logger( Dht22SerialService.name );
  ...
  getDeviceTypeName(): string {
    return 'Temperature/Humidity Sensor DHT22 (Serial)';
  }
  ...
  getSensorInfo(): void {
    this.serialConnectorService.sendMessage({ 
      ...
      requestMeta: {
        ...
        requestService: Dht22SerialService.name,
      }
    });
  }
  ...
  validateResponse( message: any ): boolean {
    ...
    if( message.requestMeta.requestService !== Dht22SerialService.name ) {
