...
class Bme680SerialDevice extends Device {
  address!: string;
  id!: string;
  interval!: number;
  parent!: string;
...
class Bme680SerialCommandData {
  slug!: string;
  value!: any;
}

@Injectable()
export class Bme680SerialService implements OnModuleDestroy, VirtualDevice {

  private appMessagesService!: AppMessagesService;
  private serialConnectorService!: SerialConnectorService;

  private readonly logger = new Logger( Bme680SerialService.name );
  ...
  getDeviceTypeName(): string {
    return 'Environmental Sensor BME680 (Serial)';
  }
  ...
  getSensorInfo(): void {
    this.serialConnectorService.sendMessage({ 
      ...
      requestMeta: {
        ...
        requestService: Bme680SerialService.name,
      }
    });
  }
  ...
  validateResponse( message: any ): boolean {
    ...
    if( message.requestMeta.requestService !== Bme680SerialService.name ) {
