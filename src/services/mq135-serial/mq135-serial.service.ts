...
class Mq135SerialDevice extends Device {
  address: string;
  id: string;
  interval: number;
  parent: string;
...
class Mq135SerialCommandData {
  slug: string;
  value: any;
}

@Injectable()
export class Mq135SerialService implements OnModuleDestroy, VirtualDevice {

  private appMessagesService: AppMessagesService;
  private serialConnectorService: SerialConnectorService;

  private readonly logger = new Logger( Mq135SerialService.name );
  ...
  getDeviceTypeName(): string {
    return 'Air Quality Sensor MQ135 (Serial)';
  }
  ...
  getSensorInfo(): void {
    this.serialConnectorService.sendMessage({ 
      ...
      requestMeta: {
        ...
        requestService: Mq135SerialService.name,
      }
    });
  }
  ...
  setDevice( device: Mq135SerialDevice ): void {
  ...
  validateResponse( message: any ): boolean {
    ...
    if( message.requestMeta.requestService !== Mq135SerialService.name ) {
  ...
  updateDeviceData( device: Mq135SerialDevice ): void {
